...
\begin{figure}[!htbp] \centering \includegraphics[width=\linewidth]{map2.jpeg} \caption{Search and rescue scenario 2 with obstacles and victims} \label{fig:map2} \end{figure}

\subsubsection{SLAM Assisted Ground Robot}
The SLAM only implementation was unable to locate all the victims meaning no time could be recorded, and no result obtained.
\subsubsection{Aerial Supported Ground Robot}
The average time that the aerial supported ground robot took to completes its search and rescue mission was $116$s. It took an average of $71$s for the drone to complete the search for the victims, and the rover took an average of $45$s for it to pursue and visit all those victims.
...
\begin{itemize}
    ...
    Victim detection relies only on green colour detection, making it prone to lighting and specific camera viewpoints. This also makes the robot believe that no obstacle is coloured green and every victim has a shared shirt colour. 
...
\begin{itemize}
    \item Edward Silvey — Abstract; Introduction; Methods (Drone Movement and Flight Control; Comparing Approaches); Appendix; Referencing
    \item Alfian Fadhlurrahman — Methods (Ground Robot Navigation and Obstacle Avoidance); Conclusion (Limitations)
    \item Shailee Kampani — Methods (Victim Detection and Communication); Conclusion
    \item Alvin Afilla — Methods (Ground Robot SLAM-Assisted Navigation); Results
\end{itemize}
